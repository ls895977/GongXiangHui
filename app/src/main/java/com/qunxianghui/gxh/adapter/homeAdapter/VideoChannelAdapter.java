...
public class VideoChannelAdapter extends BaseMultiItemQuickAdapter<ChannelItem, BaseViewHolder> {

    private static Long SPACE_TIME = 100l;
    ...
    public VideoChannelAdapter(List<ChannelItem> data, ItemTouchHelper itemTouchHelper) {
        super(data);
        this.mItemTouchHelper = itemTouchHelper;
        mIsEdit = false;
        addItemType(ChannelItem.TYPE_MY, R.layout.item_channel_title);
        addItemType(ChannelItem.TYPE_MY_CHANNEL, R.layout.channel_rv_item);
        addItemType(ChannelItem.TYPE_OTHER, R.layout.item_channel_title);
        addItemType(ChannelItem.TYPE_OTHER_CHANNEL, R.layout.channel_rv_item);
    }
    ...
    private int getMyChannelSize() {
        int count = 0;
        for (ChannelItem channelItem : mData) {
            if (channelItem.getItemType() == ChannelItem.TYPE_MY_CHANNEL) count++;
        }
        return count;
    }
    ...
    private int getMyLastPosition() {
        for (int i = mData.size() - 1; i >= 0; i--) {
            if (ChannelItem.TYPE_MY_CHANNEL == mData.get(i).getItemType()) {
                return i;
            }
        }
        return -1;
    ...
    private int getOtherFirstPosition() {
        for (int i = 0; i < mData.size(); i++) {
            if (ChannelItem.TYPE_OTHER_CHANNEL == mData.get(i).getItemType()) return i;
        }
        return -1;
    }
                    ...
                    public void onClick(View v) {
                        ...
                        if (!mIsEdit && !up) {
                            ...
                            if (mRecyclerView.indexOfChild(targetView) >= 0 && otherFirstPosition != -1) {
                                ...
                                //我的频道 移动到 推荐频道的第一个
                                channel.viewType = ChannelItem.TYPE_OTHER_CHANNEL;//改为推荐频道类型
...
//                baseViewHolder.getView(R.id.tv_sort).setTag(false);
                ...
                baseViewHolder.getView(R.id.tv_channelname).setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (mRecyclerView.indexOfChild(targetView) >= 0 && myLastPosition != -1) {
                            ...
                            // 推荐频道 移动到 我的频道的最后一个
                            channel.viewType = ChannelItem.TYPE_MY_CHANNEL;//改为推荐频道类型
    ...
    private void startAnimation(final View currentView, int targetX, int targetY) {
        ...
        TranslateAnimation animator = getTranslateAnimator((float) (targetX - currentView.getLeft()), (float) (targetY - currentView.getTop()));
    ...
    private void startEditMode(Boolean isEdit) {
        ...
        for (int count = childCount; count >= 0; count--) {
            View child = mRecyclerView.getChildAt(count);
