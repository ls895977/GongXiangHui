...
public class CircleIndicatorView extends View {
    ...
    private int gap = 30;//各个横向排列的小球间距
    ...
    private int circle_normal_radius = 8; //普通小球半径
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int start_x = (w + ((gap + getNormalCircleRadius()) * (getCircleCount() - 1))) / 2;
        int r;
        for (int i = 0; i < getCircleCount(); i++) {
            if (i == translate_pos) {
                r = getSelectedCircleRadius();
                mP.setColor(getCircleSelectedColor());
            } else {
                r = getNormalCircleRadius();
                mP.setColor(getCircleUnSelectedColor());
            }

            //自右向左绘制。从最右边往左边绘制小球
            //如果从该自定制的View左边绘制，直接将 x=0即可。
            canvas.drawCircle(start_x - i * getCirlceGap() - getPadding(), h / 2, r, mP);
        }
    }
}
